...
variable "sort_key" {
  ...
  description = "Optional sort key definition. Set to null to create a table without a sort key."
  default     = null
...
variable "tags" {
  ...
  validation {
    condition     = alltrue([for key, value in var.tags : length(trimspace(key)) > 0])
